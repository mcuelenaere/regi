...
*/
/* Regi patch: replaced glib / gio-coroutine / spice-util / decode.h /
   canvas_utils (pixman) includes with a self-contained shim. The GLZ
   decoder now allocates plain malloc'd BGRA buffers and runs synchronously
   in-order (no coroutine wait). See PATCHES.md and glz_shim.h. */
#include "config.h"
...
/* Regi patch: `data` is a plain malloc'd 32bpp (BGRA) buffer of
   gross_pixels*4 bytes instead of a pixman surface. The decode templates
   and back-reference lookups (glz_decoder_window_bits) index it linearly
   in decode order, so no negative-stride / top-down pointer adjustment is
   needed here; the top_down flag is preserved in the header for the caller
   to orient the final blit. See PATCHES.md. */
struct glz_image {
                                       ...
                                       int type, void *opaque)
{
    ...
    img->hdr = *hdr;
    img->data = (uint8_t *)calloc((size_t)img->hdr.gross_pixels, 4);
    return img;
}
...
/* Regi patch: removed the now-unused wait_for_image() coroutine callback
   and its wait_for_image_data struct. See PATCHES.md. */
static void *glz_decoder_window_bits(SpiceGlzDecoderWindow *w, uint64_t id,
                                     uint32_t dist, uint32_t offset)
{
    /* Regi patch: dropped the coroutine wait. We decode display images
       synchronously and in id order, so the referenced (older) image is
       already present in the window. See PATCHES.md. */
...
/* ------------------------------------------------------------------ */
/* Regi additions: public CSpiceCodecs GLZ API. Placed here (rather than in
   regi_codecs.c) because it needs decode-glz.c's file-static structs
   (GlibGlzDecoder, struct glz_image, the static decode()). See
   CSpiceCodecs.h for the contract. */

#include "CSpiceCodecs.h"

RegiGlzWindow *regi_glz_window_new(void) { return glz_decoder_window_new(); }
void regi_glz_window_reset(RegiGlzWindow *w) { if (w) glz_decoder_window_clear(w); }
void regi_glz_window_free(RegiGlzWindow *w) { glz_decoder_window_destroy(w); }
...
int regi_glz_decode_bgra(RegiGlzWindow *w,
                         ...
                         int *out_top_down)
{
    if (!w || !data || len == 0 || !out_bgra || !out_width || !out_height || !out_top_down)
        return -1;
    ...
    /* Decodes into a window image and records the header in d.image. */
    decode(&d.base, (uint8_t *)(void *)data, (SpicePalette *)palette, NULL);

    uint64_t id = d.image.id;
    struct glz_image *img = w->images[id % w->nimages];
    if (!img || img->hdr.id != id || !img->data)
        return -1;

    uint32_t width = d.image.width, height = d.image.height;
    if (width == 0 || height == 0)
        return -1;
    ...
    if (!out)
        return -1;
    memcpy(out, img->data, sz);

    *out_bgra = out;
    *out_width = width;
    *out_height = height;
    *out_top_down = d.image.top_down ? 1 : 0;
    return 0;
}
